...
public sealed class TaskReviewWorkflow : ITaskReviewWorkflow
{
    ...
    private readonly IReviewPromptFactory _reviewPromptFactory;
    private readonly Dictionary<string, IProviderAdapter> _providerAdapters;

    public TaskReviewWorkflow(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var reviewStatus = TaskWorkflowStateMachine.StartTaskReview(snapshot.Manifest.Status);
        var reviewManifest = snapshot.Manifest with
        {
            Status = reviewStatus,
            UpdatedAtUtc = startedAtUtc,
        };

        await _taskWorkspaceStore.SaveTaskAsync(workspaceRootPath, reviewManifest, snapshot.TaskMarkdown, cancellationToken);

        var resolution = await _artifactReferenceResolver.ResolveAsync(snapshot, cancellationToken);
        var promptPackage = _reviewPromptFactory.Create(snapshot with { Manifest = reviewManifest }, resolution);
        ...
        try
        {
            ...
            var latestSnapshot = await _taskWorkspaceStore.LoadTaskAsync(workspaceRootPath, snapshot.Manifest.Id, cancellationToken)
                ?? throw new InvalidOperationException($"Task '{snapshot.Manifest.Id}' was not found after saving the review run.");

            var updatedManifest = latestSnapshot.Manifest with
            {
                Status = TaskWorkflowStateMachine.CompleteTaskReview(reviewManifest.Status),
            ...
            return new TaskReviewResult
            {
                ...
                MissingAliases = resolution.MissingAliases,
                Usage = response.Usage,
            };
        }
        catch (Exception ex)
        {
            ...
            var failureSummary = $"Task review failed: {ex.Message}";
            ...
            var latestSnapshot = await _taskWorkspaceStore.LoadTaskAsync(workspaceRootPath, snapshot.Manifest.Id, cancellationToken)
                ?? throw new InvalidOperationException($"Task '{snapshot.Manifest.Id}' was not found after saving the failed review run.");
    ...
    private static string BuildPromptMarkdown(ReviewPromptPackage promptPackage)
    {
    ...
    private static string BuildSummary(string outputText, TaskReferenceResolution resolution)
    {
        ...
        {
            return $"Review completed with missing references: {string.Join(", ", resolution.MissingAliases.Select(alias => $"@{alias}"))}.";
        }
