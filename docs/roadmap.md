...
- Current slice delivered:
  ...
  - explicit approve or revise decision UI with persisted `user-decision` runs
...
- Current slice delivered:
  ...
  - task-defined input folder declarations and optional output file targets in the task manifest
  - file and folder imports into declared task input paths
  - markdown output artifact creation under `outputs/`, including run-scoped copies and declared output targets when present
